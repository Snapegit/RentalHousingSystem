		...
		<el-dialog v-model="formVisible" :title="formTitle" width="80%" destroy-on-close :fullscreen='false'>
			<el-form class="formModel_form" ref="formRef" :model="form" label-width="$template2.back.add.form.base.labelWidth" :rules="rules">
				<el-row>
					...
					<el-col :span="24">
						<el-form-item label="月数" prop="yueshu">
							<el-input class="list_inp" v-model.number="form.yueshu" placeholder="月数"
								 type="text" 								:readonly="!isAdd||disabledForm.yueshu?true:false" />
						</el-form-item>
					</el-col>
					...
					<el-col :span="24">
						<el-form-item label="租赁时间" prop="zulinshijian">
							<el-date-picker
								class="list_date"
								v-model="form.zulinshijian"
								format="YYYY-MM-DD HH:mm:ss"
								value-format="YYYY-MM-DD HH:mm:ss"
								type="datetime"
								:readonly="!isAdd||disabledForm.zulinshijian?true:false"
								placeholder="请选择租赁时间" />
						</el-form-item>
					</el-col>
					<el-col :span="24">
						<el-form-item label="租房合同" prop="zufanghetong">
							<uploads
								:disabled="!isAdd||disabledForm.zufanghetong?true:false"
								type="file"
								action="file/upload" 
								tip="请上传租房合同" 
								:limit="1" 
								style="width: 100%;text-align: left;"
								:fileUrls="form.zufanghetong?form.zufanghetong:''" 
								@change="zufanghetongUploadSuccess">
							</uploads>
						</el-form-item>
					</el-col>
				</el-row>
...
<script setup>
	...
	const type = ref('')
	//租房合同上传回调
	const zufanghetongUploadSuccess=(e)=>{
		form.value.zufanghetong = e
	}
	const zongjia =computed(()=>{
		let c = form.value
		let a = c.zulinjiage*c.yueshu
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		resetForm()
			form.value.zulinshijian = context?.$toolUtil.getCurDateTime()
		if(formId){
			id.value = formId
			type.value = formType
		}
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='yueshu'){
					form.value.yueshu = row[x];
					disabledForm.value.yueshu = true;
					continue;
				}
				if(x=='zongjia'){
				...
				if(x=='zulinshijian'){
					form.value.zulinshijian = row[x];
					disabledForm.value.zulinshijian = true;
					continue;
				}
				if(x=='zufanghetong'){
					form.value.zufanghetong = row[x];
					disabledForm.value.zufanghetong = true;
					continue;
				}
			}
			if(row){
	...
	//提交
	const save=()=>{
		if(form.value.zufanghetong!=null) {
			form.value.zufanghetong = form.value.zufanghetong.replace(new RegExp(context?.$config.url,"g"),"");
		}
