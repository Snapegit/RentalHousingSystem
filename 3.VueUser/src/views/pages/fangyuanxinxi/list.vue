	...
	<div class="app-contain" :style='{"minHeight":"100vh","padding":"0 0 40px","margin":"0 auto","overflow":"hidden","borderRadius":"0px","background":"#fff","width":"76%","position":"relative"}'>
		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			...
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('fangyuanxinxi','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
				...
				<div class="hot_list">
					<div class="hot" v-for="(item,index) in hotList" :key="index" @click="detailClick(item.id)">
						...
						<div class="hot_content">
							<div class="hot_text">{{item.fangwumingcheng}}</div>
							<div class="hot_text">{{item.fangwudizhi}}</div>
							<div class="hot_text">{{item.zulinjiage}}</div>
						</div>
					</div>
				</div>
			</div>
...
<script setup>
	...
	const addClick = () => {
		router.push('/index/fangyuanxinxiAdd')
	}
	//判断是否从个人中心跳转
	...
	const getList = () => {
		...
		if (categoryIndex.value != -1) {
			params.fangwuleixing = categoryList.value[categoryIndex.value]
		}
		...
		if (searchQuery.value.zulinjiagestart&&searchQuery.value.zulinjiagestart!='') {
			params.zulinjiagestart = searchQuery.value.zulinjiagestart
		}
		if (searchQuery.value.zulinjiageend&&searchQuery.value.zulinjiageend!='') {
	...
	const detailClick = (id) => {
		router.push('fangyuanxinxiDetail?id=' + id + (centerType.value?'&&centerType=1':''))
	}
	//热门
	const hotList = ref([])
	const getHotList = () => {
